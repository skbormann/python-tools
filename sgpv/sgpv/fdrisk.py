...
def fdrisk(*, sgpval: int = 0, null_lo, null_hi, std_err: float,
    ...
    * : TYPE
    ...
    #import sgpv
    from sgpower import sgpower #I would to like use the sgpv-module directly, but did not find out yet how
    ...
    if null_lo == null_hi:
        if any(null_lo != null_space):
            print(colored(
                ...
                `null.space` not permitted; `null.space` set to be ', 'red'),
                  round(null_lo, 2),'.')
                ...
                if (max(null_space) > null_hi) | (min(null_space) < null_lo):
                    print(colored(
                        'null space must be inside originally specified \
                            null hypothesis; at least one null space bound \
                                has been truncated', 'red'))
        ...
        if alt_weights in ['Uniform', 'GBeta', 'TruncNormal']:
            ...
            if len(alt_space) == 2:
               ...
               # if np.any(alt_space > null_lo and alt_space < null_hi): The original approach
                if np.any(alt_space > null_lo) & np.any(alt_space < null_hi): #not sure if this approach yields always the same results
